# -*- coding: utf-8 -*-
"""
Title: Abstraction Heirarchy Tool
...
class MyApp(Tk):  
    def __init__(self):
        ...
        self.title('Abstraction Heirarchy Tool')
        ...
        self.addStatsToSpreadsheet = StringVar()
    ...
    def importData(self):
        #Input a file, Load the data files into pandas
        try:
            if '.xlsx' in self.openPath:
                self.loadedData = pd.read_excel(self.openPath, header=0)
            else:
                self.loadedData = pd.read_csv(self.openPath, header=0)
        except:
            pass
...
class MyWindow(ttk.Frame):
    def __init__(self,master):
        ...
        self.master.appTopFile.add_command(label='Save Data to Excel', command=self.saveDataExcel)
        self.master.appTopFile.add_command(label='Save Data to CSV', command=self.saveDataCSV)
        self.master.appTopFile.add_separator()
        ...
        self.master.appTopStatistics.add_checkbutton(label='Add All Statistics to Excel', variable=self.master.addStatsToSpreadsheet, onvalue=1,offvalue=0)
    ...
    def openWindow(self):
        self.master.openPath = filedialog.askopenfilename(title='Open a file',filetypes=[("Excel Files",'*.xlsx'),("CSV Files",'*.csv')])
    ...
    def saveDataExcel(self):
        ...
        # Menu Function for Saving Data to CSV
    def saveDataCSV(self):
        ...
        # Output the new dataframe to excel
        self.master.savePath = filedialog.asksaveasfilename(title='Save a file',
                                                            filetypes=[("CSV Files", '*.csv')])
        if '.csv' not in self.master.savePath:
            self.master.savePath = self.master.savePath + '.csv'
        self.master.loadedData.to_csv(self.master.savePath)
